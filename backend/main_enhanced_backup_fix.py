...
# PyTorch compatibility fix for XTTS v2 model loading
...
class TTSService:
    ...
    def load_coqui_models(self):
        ...
        try:
            ...
            os.environ['COQUI_TTS_NO_MECAB'] = '1'  # Add this to prevent MeCab loading
            ...
            try:
                ...
                if not os.path.exists(mecabrc_path):
                    ...
                    logger.info(f"✅ Created dummy mecabrc at {mecabrc_path}")
            except Exception as e:
                logger.warning(f"⚠️ Could not create dummy mecabrc: {e}")
            ...
            try:
                # Use a more targeted approach to avoid importing Japanese phonemizer
                # which requires MeCab
                import importlib
                import TTS.api
                
                # Try multiple Portuguese models in order of preference
                models_to_try = [
                    "tts_models/pt/cv/vits",  # Portuguese VITS model
    ...
    def load_xtts_v2_model(self):
        """Explicitly load XTTS v2 model for better voice cloning"""
        try:
            logger.info("🔄 Explicitly loading XTTS v2 model...")
    ...
    def load_coqui_models(self):
        """Load Coqui TTS models with Windows MeCab fix using monkey patch"""
        try:
            # Set environment variables to bypass MeCab issues on Windows
            import os
            os.environ['MECAB_PATH'] = ''
            ...
            # Try XTTS v2 model first (best for voice cloning), then other models
            models_to_try = [
                "tts_models/multilingual/multi-dataset/xtts_v2",  # XTTS v2 - Best for voice cloning
                "tts_models/multilingual/multi-dataset/your_tts",  # YourTTS - Multilingual with voice cloning
                "tts_models/pt/cv/vits",  # Portuguese VITS model
                "tts_models/pt/cv/tacotron2-DDC",  # Portuguese Tacotron2
                "tts_models/en/ljspeech/tacotron2-DDC"  # English fallback
            ]
    ...
    async def synthesize_speech(self, text: str, voice: str = None, 
        ...
        try:
            ...
            if engine == "coqui" and self.coqui_tts:
                ...
                # Try to reload Coqui TTS
                logger.info("🔄 Attempting to reload Coqui TTS...")
                self.load_coqui_models()
                
                if self.coqui_tts:
                    logger.info("✅ Coqui TTS reloaded successfully")
                    # Retry the synthesis
                    return await self.synthesize_speech(text, voice, speed, language, engine)
                else:
                    logger.error("❌ Coqui TTS still not available after reload attempt")
                    return {"success": False, "error": "Coqui TTS not available - likely due to MeCab issues on Windows. Try using pyttsx3 or gtts engines instead."}
    ...
    try:
        ...
        coqui_models = [
            {
                ...
                "type": "neural",
                "quality": "highest",
                "description": "Modelo XTTS v2 multilingual com suporte avançado para clonagem de voz",
                "gender": "adaptable",
                "voice_cloning": True
            },
            {
        ...
        return {
            ...
            "voice_cloning_info": {
                ...
                "max_file_size": "25MB",
                "language_note": "Modelos treinados principalmente em português brasileiro",
                "accent_note": "Sotaque europeu pode soar diferente",
                "quality_note": "Vozes clonadas usam tecnologia de ponta mas não são perfeitas",
                "improvement_guide": "Veja CLONED_VOICES_IMPROVEMENT_GUIDE.md para dicas"
            },
            "engines": {
                "coqui": {
                    "name": "Coqui TTS",
...
async def train_voice_clone(request: dict):
    ...
    try:
        ...
        if reference_file and os.path.exists(ref_audio_dir):
            # Look for the specific reference file
            ref_audio_path = os.path.join(ref_audio_dir, reference_file)
            if not os.path.exists(ref_audio_path):
                ref_audio_path = None
        
        # If no specific file, look for any file matching the voice name
        if not ref_audio_path and os.path.exists(ref_audio_dir):
            for filename in os.listdir(ref_audio_dir):
                if voice_name.lower() in filename.lower() and filename.endswith(('.wav', '.mp3', '.flac')):
                    ref_audio_path = os.path.join(ref_audio_dir, filename)
                    break
        
        if not ref_audio_path:
            logger.warning(f"⚠️ No reference audio found for voice '{voice_name}'")
            # Create a basic config without training
            voice_config = {
                "name": voice_name,
                "display_name": voice_name,
                "created_at": datetime.now().isoformat(),
                "status": "simulated",
                "description": f"Voz clonada simulada: {voice_name}",
                "language": "pt-BR",
                "gender": "unknown",
                "reference_file": None,
            ...
            # For now, we'll just create a config with the reference info
            voice_config = {
                "name": voice_name,
                "display_name": voice_name,
                "created_at": datetime.now().isoformat(),
                ...
                "language": "pt-BR",
                "gender": "unknown",
                "reference_file": os.path.basename(ref_audio_path),
                "model_type": "your_tts",
    ...
    async def clone_voice(
        ...
        if os.path.exists(config_path):
            try:
                with open(config_path, 'r', encoding='utf-8') as f:
                    config = json.load(f)
                is_real_clone = config.get("status") == "trained"
        ...
        result = await tts_service.synthesize_speech(
            ...
            engine="coqui" if tts_service.coqui_tts else "gtts"
        )
        
        if result["success"]:
            return {
                "success": True,
                "message": f"Test completed for cloned voice '{voice_name}'",
                "voice_name": voice_name,
                "text": text,
                "is_real_clone": is_real_clone,
...
async def test_cloned_voice(request: dict):
    ...
    try:
        ...
        # Check if this is a real cloned voice or simulated
        voices_dir = os.path.join(os.getcwd(), "cloned_voices")
